...
const DiscoverSection = ({ propertyType = 'Home', content, locale = "en" }) => {
  const fallbackContent =
    propertyType === "Home" || propertyType === "منزل" ? fallbackHomeSolutions?.discover : fallbackCommercialSolutions?.discover;
  const discoverContent = content || fallbackContent;
  const isHome = propertyType === 'Home' || propertyType === 'منزل';
  ...
  return (
    <section className="my-[1.5rem] md:my-[8rem] relative overflow-hidden">
      ...
      <div className="flex justify-end relative bg-[var(--secondary)] w-full h-[10rem] sm:h-[15rem] md:h-[22rem] lg:h-[25rem] xl:h-[28rem]">
        <div className="absolute top-[50%] start-0 -translate-y-1/2 h-full">
          <div className="absolute bg-white rounded-full w-[13rem] sm:w-[25rem] md:w-[30.5rem] lg:w-[40.5rem] h-[120%] top-[50%] translate-y-[-50%] -start-1/2"></div>
          <Image
            src={imageSrc}
            alt={imageAlt}
            width={400}
            ...
            className="relative w-[8rem] sm:w-[15rem] md:w-[19rem] lg:w-[25rem] h-full object-cover rounded-e-[50%]"
          />
        </div>
        ...
        <div className="w-1/2 sm:w-3/5 md:w-2/4 lg:w-2/3 flex flex-col justify-center items-start lg:ps-[10rem] xl:ps-[5rem] relative z-10">
          <h4 className="text-white text-xs sm:text-2xl lg:text-[2.5rem] font-bold mb-[1rem] md:mb-[2rem] text-start">
